...
BEGIN
	...
	END + 
	CASE 
		WHEN @QuantityRecordsAffected IS NOT NULL 
		THEN CAST (@QuantityRecordsAffected AS VARCHAR(20)) + 
		' records affected. ' 
		ELSE '' 
	END + ' ('+ CONVERT(VARCHAR(20),CURRENT_TIMESTAMP,120) + '). ' ; 
SET
	@StatusMessage = ISNULL(@StatusMessage,'') + @TextToAppend; RETURN(
	@StatusMessage) 
END
GO
/*
